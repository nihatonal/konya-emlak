    ...
    <Container className="py-16">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mt-5">
        {latestBlogs.map(blog => (
          <article
            key={blog.id}
            className="flex flex-col h-full border border-1 hover:border-bvs-darkGreen/50 rounded-lg overflow-hidden transition"
          >
            {/* Görsel */}
            ...
            <div className="bg-white p-5 flex flex-col flex-grow">
              {/* Etiketler ve tarih */}
              <div className="text-xs flex items-center gap-5 mb-2">
                <div className="flex items-center gap-2 relative group cursor-pointer">
                  {(blog.tags?.length ? blog.tags : defaultTags[language] || defaultTags.tr)
                    .slice(0, 2)
                    .map((tag, index) => (
                      <p
                        key={index}
                ...
                <p className="flex items-center gap-1 text-bvs-lightGreen relative group hover:text-bvs-darkGreen transition">
              ...
              <NavLink
                to={`blog/${blog.slug}`}
                className="text-base font-semibold tracking-wide mt-2 mb-4 line-clamp-2 hover:text-bvs-darkGreen transition"
              >
                {blog?.title}
              </NavLink>
              ...
              <NavLink
                to={`blog/${blog.slug}`}
                className="mt-4 inline-block text-sm text-white bg-bvs-darkGreen hover:bg-bvs-midGreen transition px-4 py-2 rounded-lg text-center"
              >
                Devamını Oku
              </NavLink>
            </div>
